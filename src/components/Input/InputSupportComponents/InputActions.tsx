...
import {InputActionsStyledContainer, InputActionStyledItem} from "./InputActions.styled";
...
export const InputActions: FC<InputActionsProps> = ({actions, actionHandler, onDeleteAction}) => {
	if (actions && actionHandler) {
		const deleteActionHandler = (e: React.MouseEvent<HTMLSpanElement>, action: TextInputAdditionalAction) => {
			e.stopPropagation()
				...
				{actions.map((action, index) => (
					...
					>
						...
						<InputActionDeleteElement
							onDelete={onDeleteAction && ((e) => deleteActionHandler(e, action))}
						/>
					</InputActionStyledItem>
				))}
			</InputActionsStyledContainer>
