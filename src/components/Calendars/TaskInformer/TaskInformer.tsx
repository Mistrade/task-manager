...
const TaskInformerMain: FC<TaskInformerMainProps> = ({taskItem}) => {
	const options = useMemo(() => {
		...
		const end = dayjs(taskItem.timeEnd)
	...
	return (
		...
		>
			<FlexBlock direction={'row'} width={'100%'} gap={12}>
                ...
                &:hover {
                  background-color: ${disabledColor};
						...
						>
							<ArrowIndicator
								priorityKey={taskItem.priority}
								isCompleted={taskItem.status === 'completed'}
							/>
						</FlexBlock>
						<FlexBlock direction={'column'} gap={8} width={'100%'}>
							<FlexBlock width={'100%'} mb={4}>
								<Heading.H2>{taskItem.title}</Heading.H2>
							</FlexBlock>
                    ...
                    &:hover {
                      background-color: ${disabledColor};
									...
									`}
								>
									<EventIcon status={taskItem.status} size={20}/>
								</FlexBlock>
								{convertEventStatus(taskItem.status)}
							</FlexBlock>
							<FlexBlock>
			...
			<FlexBlock>
				Событие создано:
				{dayjs(taskItem.createdAt).format(`${DATE_RENDER_FORMAT} по местному времени`)}
			</FlexBlock>
		</FlexBlock>
	)
}
