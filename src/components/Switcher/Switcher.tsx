...
export function Switcher<T extends string = string>(props: SwitcherProps<T>) {
	return (
		<FlexBlock borderBottom={`1px solid ${disabledColor}`} justify={'space-between'} align={'flex-end'} height={45}>
			<FlexBlock>
				{props.switchersList.map((item) => (
					...
					{item.title}
						...
						{(props.badges && props.badges[item.type] && props.badges[item.type] > 0) ? (
							<Badge
								style={{marginLeft: 4}}>{props.badges[item.type]}</Badge>
						) : <></>}
					</SwitchCalendarModeTab>
				))}
			</FlexBlock>
