...
import {CalendarTaskItem, EventsStorage} from "../../../pages/Planner/planner.types";
		...
		return {
			removeGroup: mutation<MyServerResponse, GroupIdObject>({
				...
				invalidatesTags: (result, error, arg, meta) => !error
					? ['Groups', 'Events', 'EventsScheme', 'EventsCount']
					: []
			}),
			...
			getGroupsList: query<MyServerResponse<ArrayOfGroupModel>, GetGroupsListRequestProps>({
				query: (args) => ({
			...
			changeSelectGroup: mutation<MyServerResponse, ChangeSelectGroupRequestProps>({
				query: (args) => ({
				...
				invalidatesTags: (result, error, arg, meta) => !error
					? ['Events', 'Groups', 'EventsCount', "EventsScheme"]
					: []
			}),
			...
			getShortEventsArray: query<SortedEventsObject, GetEventsFiltersRequestProps>({
				query: (props) => ({
				...
				providesTags: ['Events'],
				transformResponse(value: MyServerResponse<ShortEventsArray>, meta, arg) {
			...
			getEventsStorage: query<MyServerResponse<EventsStorage>, GetEventsFiltersRequestProps>({
				query: (props) => ({
			...
			createEvent: mutation<MyServerResponse<EventIdObject>, CalendarTaskItem>({
				...
				invalidatesTags: (result, error, arg, meta) => !error
					? ['Events', 'EventsScheme', 'EventsCount', "Chains", "EventHistory"]
					: []
			}),
			...
			getEventsScheme: query<GetEventsSchemeResponse, GetEventsFiltersRequestProps>({
				query: (args) => ({
			...
			updateTask: mutation<MyServerResponse, UpdateEventRequestProps>({
				...
				invalidatesTags: (result, error, arg, meta) => {
					...
					if (arg.field === 'time' || arg.field === 'timeEnd' || arg.field === 'group') {
						return ['EventInfo', 'Events', 'EventsCount', 'EventsScheme', 'Chains', "EventHistory"]
					}
					
					if (arg.field === 'status') {
						return ['EventInfo', 'EventsCount', 'Events', 'Chains', "EventHistory"]
					}
					
					return ['EventInfo', 'Events', 'Chains', "EventHistory"]
				}
			}),
			getEventsCountOfStatus: query<MyServerResponse<SwitcherBadges<EventFilterTaskStatuses>>, GetEventsFiltersRequestProps>({
				query: (args) => ({
			...
			toggleIsImportantCommentState: mutation<MyServerResponse, UpdateCommentIsImportantRequestData>({
				query: (data) => ({
