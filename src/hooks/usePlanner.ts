...
export interface UsePlannerReturned {
	...
	setCreateEventDate: (value: Date | null, initialValues?: EventInfoModel | null) => void,
	...
	onSuccessClonedEvent: (date: Date, taskStatus: EventFilterTaskStatuses, taskId?: ObjectId) => void,
...
export const usePlanner: UsePlannerType = () => {
	...
	const {groupRemoved, dateOfCreateEvent, clonedEventInfo, statuses} = useAppSelector(state => state.planner)
	
	const dispatch = useAppDispatch()
	
	const setCreateEventDate = useCallback((value: Date | null, initialValues?: Partial<EventInfoModel> | null) => {
	...
	const onCreateGroup: UsePlannerReturned['onCreateGroup'] = useCallback((calendarId) => {
	...
	const onSelectRemovedGroup = useCallback((item: GroupModelResponse | null) => {
	...
	const onChangePlanner: UsePlannerReturned['onChangePlanner'] = useCallback((date, l) => {
		navigate(`/planner/${l}/${statuses}`, {replace: true})
		if ('fromDate' in date || 'toDate' in date) {
			dispatch(changePlanner({
				layout: date.layout,
				date: {
					layout: date.layout,
	...
	const onCloseCreateEventModal = useCallback((taskStatus?: EventFilterTaskStatuses, taskId?: ObjectId) => {
		setCreateEventDate(null, null)
		const defaultPath = `/planner/${planner.layout}/${taskStatus || statuses}`
		navigate(taskId ? `${defaultPath}/${taskId}` : defaultPath, {replace: true})
	}, [planner.layout, statuses])
	
	const onCloneEvent = useCallback((event: Partial<EventInfoModel>) => {
	...
	const onSuccessClonedEvent: UsePlannerReturned['onSuccessClonedEvent'] = useCallback((date, taskStatus, taskId) => {
		dispatch(changeEventStatuses(taskStatus))
		dispatch(changePlanner({layout: planner.layout, date: date.toString()}))
