...
export abstract class DateHelper {
  public static getHumanizeDateValue(
    date: Date | string,
    options: HumanizeDateValueOptions = {}
  ) {
    const {
      withTime = true,
      withYear = true,
    ...
    if (withYear) {
      format.push(yearPattern === 'short' ? 'YYг.' : 'YYYY');
    }
  ...
  public static checkThatDateIsDisabledOfMinDate(
    dateForCheck: Date,
    minDate?: CalendarDisabledOptions['min'],
    includeMinDate?: CalendarDisabledOptions['includeMin']
  ): boolean {
    if (!minDate) {
      return false;
    }
  ...
  public static checkThatDateIsDisabledOfMaxDate(
    dateForCheck: Date,
    maxDate?: CalendarDisabledOptions['max'],
    includeMaxDate?: CalendarDisabledOptions['includeMax']
  ): boolean {
    if (!maxDate) {
      return false;
    }
  ...
  public static checkThatDateIsWeekend(
    dateForCheck: Date,
    excludeWeekends?: CalendarDisabledOptions['excludeWeekends']
  ): boolean {
    if (!excludeWeekends) {
      return false;
    }
  ...
  public static checkThatDateIncludesInDisableDateList(
    dateForCheck: Date,
    disabledDateList?: CalendarDisabledOptions['disableDates']
  ): boolean {
    if (!disabledDateList?.length) {
      return false;
    }

    return disabledDateList.some((disabledDate) =>
      dayjs(dateForCheck).isSame(disabledDate, 'date')
    );
  }

  public static checkIsDisabled(
    dateForCheck: Date,
    ...
    return [
      min
        ? this.checkThatDateIsDisabledOfMinDate(dateForCheck, min, includeMin)
        : false,
      max
        ? this.checkThatDateIsDisabledOfMaxDate(dateForCheck, max, includeMax)
        : false,
      excludeWeekends
        ? this.checkThatDateIsWeekend(dateForCheck, excludeWeekends)
        : false,
      !!disableDates.length
        ? this.checkThatDateIncludesInDisableDateList(
            dateForCheck,
            disableDates
          )
        : false,
  ...
  public static checkThatDateInCurrentScope(
    dateForCheck: Date,
    ...
    if (context.month) {
      ...
      return (
        dateForCheck.getMonth() === ctx.month() &&
        dateForCheck.getFullYear() === ctx.year()
      );
    }
  ...
  public static createContext(
    date: Date,
    pattern: 'year' | 'month' | 'week'
  ): CalendarCurrentContext | undefined {
    if (pattern === 'year') {
      return {
...
export const getDateDescription = (
  ...
  return DateHelper.getHumanizeDateValue(d.toDate(), {
    withTime,
    monthPattern: 'full',
  });
};