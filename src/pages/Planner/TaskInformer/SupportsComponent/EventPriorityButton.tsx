...
import {darkColor, PRIORITY_TITLES, TASK_STATUSES} from "../../../../common/constants";
...
export const EventPriorityButton: FC<EventPriorityButtonProps> = forwardRef<HTMLButtonElement, EventPriorityButtonProps>(
	(props, ref) => {
				...
				<Tooltip
					content={
						props.tooltipInfo && props.tooltipInfo !== 'default'
							? props.tooltipInfo
							: `Приоритет события: ${PRIORITY_TITLES[props.priority || 'not_selected']}`
					}
				>
					<EmptyButtonStyled
					...
					>
						<FlexBlock gap={6} align={'center'} fSize={15} style={{color: darkColor}}>
							<PriorityCalendarIcon
								{...(props.iconProps || {})}
								priorityKey={props.priority || "not_selected"}
							/>
							{props.renderText && props.priority && PRIORITY_TITLES[props.priority] + ' приоритет' || ""}
						</FlexBlock>
					</EmptyButtonStyled>
				</Tooltip>
			)
