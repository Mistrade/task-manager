...
import {useGetEventInfoQuery, useRemoveEventMutation, useUpdateTaskMutation} from "../../../store/api/planning-api";
...
export const TaskInfoModal: FC<EventInfoModalProps> = ({onClose, onCloneEvent, onOpenClonedEvent}) => {
	
	const {taskId} = useParams<{ taskId: string }>()
	const {data: taskInfo, isLoading, error} = useGetEventInfoQuery(
		taskId || "",
		{
					...
					<Loader title={'Загрузка информации...'} isActive={isLoading}>
						<React.Suspense fallback={<Loader title={'Загрузка дополнительных скриптов...'} isActive={true}/>}>
							<Informer
								...
								eventErrorInfo={(error && 'data' in error && error.data.info?.message) || ""}
							/>
						</React.Suspense>
					</Loader>
				</ErrorBoundary>
