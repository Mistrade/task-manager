...
export const RemoveGroupHock: FC<RemoveGroupHockProps> = (props) => {
	if (props.groupInfo !== null) {
		return <RemoveGroupModal {...props} count={0} groupInfo={props.groupInfo}/>
	}
	
	return <></>
}

export const RemoveGroupModal: FC<RemoveGroupModalProps> = ({groupInfo, count, onSuccess, onClose}) => {
			...
			<ModalBody>
				<FlexBlock p={20} direction={'column'} width={'100%'}>
					<FlexBlock
						...
						direction={'column'}
					>
						Вы уверены, что хотите удалить группу событий: "{groupInfo.title}"?<br/>
						<strong>Все события, закрепленные за данной группой, будут так же удалены.</strong>
					</FlexBlock>
				</FlexBlock>
			</ModalBody>
			<ModalFooter>
