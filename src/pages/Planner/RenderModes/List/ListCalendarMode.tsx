...
import {ListCalendarModeProps, EventsStorage} from "../../planner.types";
...
export const ListCalendarMode: FC<ListCalendarModeProps> = ({current, onSelectTask}) => {
	...
	const onChangeDate = useCallback((date: Date | null, type: 'start' | 'end') => {
		if (type === 'start') {
			handlers.start(date)
			return dispatch(changePlanner({
				layout: 'list',
				date: {
					layout: 'list',
