...
const CreateEventForm = React.lazy(() => import('./CreateEventForm')
	.then(({CreateEventForm}) => ({default: CreateEventForm}))
)
...
export const CreateEventModal: FC<CreateEventModalProps> = ({
	...
	const {data: groupsList, isLoading} = useGetGroupsListQuery({exclude: ['Invite']})
	const getInitialValues = (data: Partial<EventInfoModel> | null | undefined): CalendarTaskItem | undefined => {
		if (!data) {
			return undefined
		}
	...
	return (
		...
		>
			<ModalBody>
				<FlexBlock minWidth={'60vw'} maxWidth={'80vw'} grow={10} height={'100%'}>
					<ErrorBoundary
					...
					>
						<Loader isActive={isLoading} title={'Загрузка данных...'}>
							<React.Suspense fallback={<Loader isActive={true} title={'Загрузка формы...'}/>}>
								<CreateEventForm
