...
export const PlannerPage: FC<CalendarProps> = ({
  ...
  const changeCurrentObserver = (
    ...
    switch (layout) {
      case 'year':
        return setYearItem((prev) =>
          observer.getYearItem(prev, new Date(current.year, 0, 1))
        );
      case 'month':
        return setMonthItem((prev) =>
          observer.getMonthItem(prev, new Date(current.year, current.month))
        );
      case 'week':
        return setWeekItem(observer.getWeekItem(current.aroundDate));
      case 'day':
        return setDateItem((prev) => observer.getDateItem(prev, current));
    }
  };
  ...
  const checkMonthItemSettingsPanel = (current: PlannerMode): MonthItem => {
    switch (current.layout) {
      ...
      case 'week':
        return new DateListGenerator({ useOtherDays: true }).getMonthItem(
          current.aroundDate
        );
      case 'month':
        return monthItem;
  ...
  return (
    <PlannerContext.Provider value={calendar}>
      <FlexBlock
        position={'relative'}
        align={'flex-start'}
        direction={'column'}
          ...
          <Routes>
            <Route path={'group'}>
              <Route
                ...
                element={
                  <UpdateGroupInfoHock
                    onClose={calendar.onCloseCreateGroupModal}
                  />
                }
              />
            </Route>
